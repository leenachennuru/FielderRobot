...
USnew = xlsread('UStestnewmat.xls');
D  = xlsread('Dtestmat.xls');
D = D';
[USna, USnb] = size(USnew);
...
V = a+(b-a)*rand(J, I);     %Randomly initialize weights matrix for hidden layer
...
while E>Emax
    ...
    while(p <= P)
        ...
        for k=1:K
            M = 0.5*(D(k,p)-O(k))^2;
            E = M + E;                              %Error for current sample 
        end
